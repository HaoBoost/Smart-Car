 ...
 * @param    Pixle: 图片数据
 * @return   无
 * @see      lq_tft18_drv_image(0, 0, 100, 63, gImage_2);
 ********************************************************************************/
void lq_tft18_drv_image(uint8_t x, uint8_t y, uint8_t w, uint8_t h, const unsigned char* Pixle)
{
    ...
    for (uint8_t i = 0; i < h; i++)
    {
        ...
        {
            if (j+x >= tft18_spi.width) break;

            color = (Pixle[(i*w + j)*2  + 1] << 8) | Pixle[(i*w + j)*2];
            lq_tft18_drv_data_mod(j+x, i+y, (lq_display_color_t)color);
        }
        if (i+y >= tft18_spi.height) break;
    }
    ioctl(tft18_spi.fd, IOCTL_TFT18_FLUSH);
}
 ...
 ********************************************************************************/
void lq_tft18_drv_cstr(uint8_t x, uint8_t y, const char *s_dat, const lq_display_font_t &font, lq_display_color_t word_color, lq_display_color_t back_color)
{
    // ========== 字库合法性校验，防止崩溃 ==========
    if(s_dat == nullptr || font.font_idx == nullptr || font.font_data == nullptr || font.font_size < 10U) return;

#if TFT18_HANZI_UTF_8
    uint8_t hanzi_len = 3;
#else
    uint8_t hanzi_len = 2;
#endif
        ...
        if (adder != 1U) // 显示汉字
        {
            ...
            for (uint16_t i = 0; i < (all_num / hanzi_num); i++)
            {
                ...
                for (uint8_t k = 0; k < poll; k++)
                {
                    ...
                    if ((user_x < tft18_spi.width) && (user_y < tft18_spi.height))
                    {
                        lq_tft18_drv_data_mod(user_x, user_y, (font.font_data[adder + 10] & (0x80U >> k)) ? word_color : back_color);
                    }
                    ls++;
                }
                adder++;
