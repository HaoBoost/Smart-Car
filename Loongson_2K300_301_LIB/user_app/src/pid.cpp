...
// 定义结构体
//  PID servo_pid;
PID Lmotor_PID;    // 左电机速度环PID
PID Rmotor_PID;    // 右电机速度环PID
PID Angle_PID;     // 角速度环PID（纯位置式，无前馈）
PID Photo_PID;     // 图像环PID（位置式）
PD_FF Photo_PID_F; // 图像环PD+前馈（转向用）
...
void PD_FF_Cal(PD_FF *pd, float target, float actual)
{
    ...
    // 输出限幅
    if (pd->output > pd->maxOutput)
        pd->output = pd->maxOutput;
    else if (pd->output < -pd->maxOutput)
        pd->output = -pd->maxOutput;
