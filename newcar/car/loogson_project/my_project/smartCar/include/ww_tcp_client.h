 ...
 *
 * @note        使用 select 实现非阻塞接收
 ******************************************************************/
    int recv_bytes(void* buffer, size_t len, int timeout_ms = 0);
 ...
 *               float values[10];
 *               int count = vofa.recv_firewater(name, values, 10);
 *               if (count > 0) {
 ...
 *******************************************************************/
    int recv_firewater(char* name, float* values, int max_values);
