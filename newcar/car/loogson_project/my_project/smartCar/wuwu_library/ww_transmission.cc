...
// 静态成员初始化
TransmissionStreamServer* TransmissionStreamServer::instance = nullptr;
...
{   
    // 获取当前原图和 JPEG 备份。
    // 如果调用的是 update_frame_mat，则优先使用原图 current_frame。
    // 如果调用的是 update_frame_jpeg，则只有从 current_jpeg 解析。
    pthread_mutex_lock(&frame_mutex);
    ...
    if (frame_copy.empty()) {
        ...
        // 如果没有 RGB 原始帧（说明只用了纯图传方案），则从保存的 JPEG 解码为 Mat。
    ...
    // 编码为 PNG 无损格式，避免 JPEG 二次压缩带来质量损失。
    ...
    std::cout << "✓ 已发送无损PNG图片到客户端: " << filename 
              << " (大小: " << png_buffer.size() / 1024 << " KB, 优先原图)" << std::endl;
}
 ...
 * @param       sig             信号值
 ******************************************************************/
void TransmissionStreamServer::signal_handler(int sig)
{
    std::cout << "\n正在关闭服务器..." << std::endl;
    if (instance) {
